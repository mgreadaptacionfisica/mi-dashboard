...
| `ventas` | Pipeline comercial (leads), desde que se agenda una llamada hasta que se gana/pierde. Etapas: `agendada → realizada → seguimiento → ganada/perdida`. `historial_llamadas` (migración 54) guarda cada intento de llamada `{fecha, hora, resultado, registradoEn}` y solo crece: `fecha_agenda`/`resultado_llamada` son el estado actual y se sobrescriben al reagendar, así que el histórico vive ahí. |
...
| `ads_kpi`, `ads_notas_mensuales`, `anuncios` | KPIs e inversión en Meta/Instagram Ads. **Sin uso desde sep-2026**: se quitó la pestaña KPI Ads; las tablas se conservan con sus datos. |
...
Quitado en sep-2026 por falta de uso (código borrado; **las tablas siguen en Supabase con sus datos**, por si se quiere recuperar algo desde git): la sección **Proyectos** (`proyectos`, `proyecto_pasos`), y en Ventas las pestañas **Setting de Instagram** (`setting_instagram`) y **KPI Ads** (`ads_kpi`, `ads_notas_mensuales`, `anuncios`). También se dejó de cargar el catálogo compartido `objetivos_fase`, que ya no se pasaba a ningún componente, y se borraron los huérfanos `AdminLogin.jsx` y `src/api/index.js`.
